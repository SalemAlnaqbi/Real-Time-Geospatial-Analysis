...
> 📍 [**View the live interactive map**](https://htmlpreview.github.io/?https://raw.githubusercontent.com/SalemAlnaqbi/Real-Time-Geospatial-Analysis/main/pilot_map.html) (GitHub renders `.html` files as source code, not as pages, so this link goes through [htmlpreview.github.io](https://htmlpreview.github.io/) to actually run it). Or download [`pilot_map.html`](pilot_map.html) and open it locally, or run `python pilot_map.py` to regenerate it.
>
> ⚠️ The map does **not** render inside the notebook on GitHub, because GitHub strips the `<script>`/`<iframe>` tags that Folium's interactive maps require from notebook outputs for security reasons. This is a GitHub limitation, not a bug in the notebook; the `Map_Gif.gif` above and the live link are the workarounds.
...
This repository contains a research project proposal for a **Data Mining and Text Analytics** course, together with a runnable pilot implementation that proves the core pipeline works end to end.

Most published climate-discourse NLP research relies on Twitter/X data and English-only text, giving a static, single-platform, single-language view of a global conversation. This project targets the gap directly: it collects **multilingual** climate discourse from **Telegram**, the dominant civic messaging platform across the GCC, Eastern Europe, and South-East Asia, combines it with mainstream news coverage, and visualises how climate framing and sentiment vary **geographically**, updated from live data rather than a fixed snapshot.
...
- 📝 **Fully documented pilot**: every design decision, surprise, and error is logged with cause and resolution in the notebook
...
- 6 to 7 countries with derivable geographic origin: United Kingdom, United States, Australia, Germany, Canada, India (plus an illustrative UAE entry in the notebook demo)
...
```bash
git clone https://github.com/SalemAlnaqbi/Real-Time-Geospatial-Analysis.git
cd Real-Time-Geospatial-Analysis
```
...
```bash
jupyter notebook "Research_Proposal_and_Pilot_Study.ipynb"
```
...
**Output:** `pilot_map.html`, which opens automatically in your browser. Reproduces the Folium choropleth map using the sentiment scores and LDA topics reported in the Pilot Study section.
...
├── pilot_map.py                               # Standalone script that regenerates the Folium map
├── pilot_map.html                             # Generated interactive map output
├── Map_Gif.gif                                # Demo animation of the interactive map
└── README.md
```
...
- **LDA over neural topic models (e.g. BERTopic):** CPU-only, deterministic given a fixed seed, and already achieves interpretable topics at pilot scale, with no GPU dependency for a real-time pipeline
...
- **Two-stage emoji handling:** emoji must be converted to text *before* tokenisation (or the tokeniser shreds them into byte fragments), and the resulting descriptions must then be filtered (or they pollute topic keywords). See the error analysis in the notebook.
...
Full methodology, tuning steps, and reasoning are documented in the [notebook](Research_Proposal_and_Pilot_Study.ipynb).
...
```bash
python pilot_map.py
# → Map saved to: pilot_map.html (opens automatically)
```

📍 [**View the live map**](https://htmlpreview.github.io/?https://raw.githubusercontent.com/SalemAlnaqbi/Real-Time-Geospatial-Analysis/main/pilot_map.html) without cloning the repo.
...
Full risk analysis (three risks + mitigations per phase) is documented in the [notebook](Research_Proposal_and_Pilot_Study.ipynb).
...
- Data Mining and Text Analytics course project
...
*Full reference list available in the [notebook](Research_Proposal_and_Pilot_Study.ipynb).*
