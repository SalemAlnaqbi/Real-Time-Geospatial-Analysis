...
>
> ⚠️ The map does **not** render inside the notebook on GitHub — GitHub strips the `<script>`/`<iframe>` tags that Folium's interactive maps require from notebook outputs for security reasons. This is a GitHub limitation, not a bug in the notebook; the `Map_Gif.gif` above and the live link are the workarounds.
...
**Output:** `pilot_map.html` — opens automatically in your browser. Reproduces the Folium choropleth map using the sentiment scores and LDA topics reported in the Pilot Study section.
